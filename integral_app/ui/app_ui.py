...
class IntegrationApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        tk.Button(input_frame, text="Integrate", font=self.font_btn,
                  bg="#2E75B6", fg="white", relief=tk.FLAT, padx=12, pady=4,
                  command=self._on_integrate).grid(row=0, column=2, padx=(0, 6))
        ...
        self.answer_label = tk.Label(ans_frame, textvariable=self.answer_var,
                                     ...
                                     bg="#F0F0F0", wraplength=680, justify=tk.LEFT)
    ...
    def _on_integrate(self):
        ...
        self.answer_var.set(f"integral( {str(expr)} ) dx  =  {str(antiderivative)} + C")
